...
>>>>>>> 14811f58deb62d7538951cf20913810621887820
...
=======
Plotting bivariate correlations was our first step in exploring the data. For each, the entire dataset was plotted without priorty first and then also plotted with the data faceted by priority. Faceting by priorty allowed us to see how important priority was to response time as well as see that generally, those with lower income spent longer times in the queue when controlling for the priority of the call. 

```{R Bivariate Predictor Race, fig.cap="Frequency Plot of Time Spent in Queue Based on Percent of White Residents"}
>>>>>>> 14811f58deb62d7538951cf20913810621887820
...
&nbsp;
=======
We ran principal component analysis in an attempt for dimension reduction. 
...
```{r PCA, fig.cap="Principal Component Analysis: A look at the scree plot and bearings", message=TRUE, warning=TRUE }
# creating PCA model
numCalls<-calls_dummy[, -c(2:5)]
calls.PCA<-prcomp(numCalls)
summary(calls.PCA)
str(calls.PCA)
...
=======
```{r, fig.cap="Results of 10-Fold Cross Validation 6 Models"}
print(model_results)
```
...
```{r, fig.cap="Test Mean Squared Error of the Test Set"}
print(test_MSE)
```
...
=======
```{r fig.cap="Summary of Boosted Random Forest Model without including priority as a predictor."}
summary(no_priority$gbm$finalModel)
```
